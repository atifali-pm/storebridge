...
import { shopLimiter } from "@/lib/rate-limit";
...
export async function shopifyGraphQL<T>(opts: {
  ...
  return shopLimiter(opts.shop.shopDomain).run(async () => {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `Shopify GraphQL ${opts.shop.shopDomain} HTTP ${res.status}: ${body.slice(0, 500)}`,
      );
    }
